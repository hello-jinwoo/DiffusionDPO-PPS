...
# Import official validation pipeline
try:
    from utils.validation_official_pipeline import (
        OfficialFluxValidation,
        create_official_validation,
    )
    OFFICIAL_PIPELINE_AVAILABLE = True
except Exception as e:
    OFFICIAL_PIPELINE_AVAILABLE = False
    logger_init = logging.getLogger(__name__)
    logger_init.debug(f"Official pipeline not available: {e}. Using legacy validation.")
...
def setup_ppd_validation(args, accelerator, filter_mode="validation_only"):
    ...
    # Load validation response file paths from file if specified
    validation_response_files = None
    validation_user_file = getattr(args, "validation_user_file", None)

    if validation_user_file and validation_user_file.strip():
        try:
            import json
            with open(validation_user_file, 'r') as f:
                validation_response_files = json.load(f)
            logger.debug(f"Loaded {len(validation_response_files)} validation response file paths from {validation_user_file}")
            logger.debug(f"Files: {[os.path.basename(f) for f in validation_response_files]}")
        except Exception as e:
            logger.warning(f"Failed to load validation user file {validation_user_file}: {e}. Falling back to all validation users.")
            validation_response_files = None

    # Build validation dataset with response file filtering
    val_dataset = build_ppd_dataset(
        mode="validation",
        data_root=data_root,
        response_files=validation_response_files
    )

    # Apply legacy filter_mode if no explicit response_files provided
    if validation_response_files is None:
        if filter_mode == "train_only":
            ...
            logger.debug(f"Validation Dataset (train users only): {len(val_dataset)} samples")

        elif filter_mode == "validation_only":
            # Default: keep validation dataset as-is (already has validation users)
            logger.debug(f"Validation Dataset (validation users only): {len(val_dataset)} samples")

        elif filter_mode == "all":
            logger.debug(f"Validation Dataset (all users): {len(val_dataset)} samples")

        else:
            raise ValueError(f"Unknown filter_mode: {filter_mode}")
    else:
        logger.debug(f"Validation Dataset (from file): {len(val_dataset)} samples")
    ...
    if validation_seed is not None:
        ...
        logger.debug(f"Validation dataloader seeded with: {validation_seed}")
    else:
        generator = None
        logger.debug("Validation dataloader using non-deterministic shuffling")
...
def compute_niqe(images):
    ...
    try:
        ...
        if not hasattr(compute_niqe, "_niqe_metric"):
            ...
            logger.debug("NIQE metric initialized with pyiqa")
        ...
        logger.debug("pyiqa not available, using simplified NIQE implementation")
...
def generate_with_denoising_loop(
    ...
    with torch.no_grad():
        ...
        # CRITICAL FIX: Save original training timesteps before overwriting
        # set_timesteps() overwrites scheduler.timesteps with inference timesteps
        # We must restore them after validation to prevent training errors
        original_timesteps = noise_scheduler.timesteps.clone()
        original_sigmas = noise_scheduler.sigmas.clone()
        ...
        # 6. Prepare UPE embeddings for PPD injection
        adapter_mode = getattr(ppd_adapter, "mode", None) if ppd_adapter is not None else None
        upe_for_injection = None

        if ppd_adapter is not None and user_embeds is not None:
            if adapter_mode == "side_adapter":
                # Side-adapter: Pass raw user embeddings to hook
                # Hook will call ppd_adapter.forward(user_embeds, image_tokens, ...)
                logger.debug("Side-adapter mode: Passing user_embeds to attention hook")
                upe_for_injection = user_embeds.to(device=device, dtype=dtype)
            elif ppd_manager is not None and ppd_manager.is_ppd_enabled():
                # Other modes: Pre-project UPE tokens
                upe_for_injection = ppd_adapter(user_embeds)
                upe_for_injection = upe_for_injection.to(device=device, dtype=dtype)
        ...
        for i, t in enumerate(timesteps):
            ...
            if upe_for_injection is not None:
                model_kwargs["joint_attention_kwargs"] = {
                    "upe_hidden_states": upe_for_injection
                }
        ...
        # CRITICAL FIX: Restore original training timesteps
        # This prevents IndexError when training resumes after validation
        noise_scheduler.timesteps = original_timesteps
        noise_scheduler.sigmas = original_sigmas
        logger.debug(f"  - Restored training timesteps: {len(original_timesteps)} steps")
...
def run_ppd_validation(
    ...
    if validation_seed is not None and hasattr(val_dataloader, 'generator') and val_dataloader.generator is not None:
        ...
        logger.debug(f"Reset dataloader generator to seed {validation_seed}")
    ...
    with validation_rng_context(seed=validation_seed, use_step_offset=False, step=global_step):
        ...
        if ppd_adapter is not None:
            ppd_adapter.eval()
            logger.debug("PPD Adapter set to eval() mode")
        if vae is not None:
            vae.eval()
        ...
        if ppd_manager is not None:
            ...
            logger.debug("PPD Manager enabled for validation")

        # Log model states for debugging
        logger.debug("Validation Model States:")
        logger.debug(f"  FLUX model training: {unet.training}")
        if ppd_adapter is not None:
            logger.debug(f"  PPD Adapter training: {ppd_adapter.training}")
        if ppd_manager is not None:
            logger.debug(f"  PPD Manager enabled: {ppd_manager.is_ppd_enabled()}")
        ...
        try:
            ...
            logger.debug(f"LUT manager initialized with {len(lut_manager.lut_files)} LUTs from {lut_dir}")
        except Exception as e:
            logger.debug(f"Failed to initialize LUT manager: {e}. Validation will proceed without LUT-based synthetics.")
            lut_manager = None

        logger.debug(f"Validation settings: max_batches={max_validation_batches}, num_images={num_validation_images}, dataloader_len={len(val_dataloader)}")

        # Calculate total number of images to generate for progress tracking
        # Each batch generates 4 variants (prefer, non_prefer, synth1, synth2) per sample
        total_images_to_generate = min(max_validation_batches, len(val_dataloader)) * 4
        ...
        logger.debug(f"Shared ValidationMetrics instance created on {metrics_device}")

        # Initialize official validation pipeline (NEW!)
        official_validator = None
        if OFFICIAL_PIPELINE_AVAILABLE and noise_scheduler is not None:
            try:
                # Get text_encoder_2 (T5) if available
                text_encoder_2 = getattr(args, 'text_encoder_2', None)
                tokenizer_2 = getattr(args, 'tokenizer_2', None)

                official_validator = create_official_validation(
                    transformer=unet,
                    vae=vae,
                    ...
                    ppd_manager=ppd_manager  # CRITICAL: Pass PPD manager for UPE injection
                )
                logger.debug("Official FluxKontext validation pipeline initialized")
                if ppd_manager is not None:
                    logger.debug("FluxPPDAttnProcessor enabled for UPE injection")
            except Exception as e:
                logger.debug(f"Failed to initialize official pipeline: {e}. Falling back to legacy manual denoising loop.")
                official_validator = None
        else:
            if not OFFICIAL_PIPELINE_AVAILABLE:
                logger.debug("Official pipeline not available, using legacy validation")
            elif noise_scheduler is None:
                logger.debug("noise_scheduler not provided, using legacy validation")

        with torch.no_grad():
            for val_batch_idx, batch in enumerate(val_dataloader):
                if val_batch_idx >= max_validation_batches:
                    logger.debug(f"Reached max validation batches ({max_validation_batches}), stopping")
                    break

                logger.debug(f"Processing validation batch {val_batch_idx + 1}/{min(max_validation_batches, len(val_dataloader))}")
                ...
                # Generate outputs for each of the 4 inputs - WITH adapter only
                generated_images_with_adapter = []    # For row 2
                resized_images_all = []  # Store resized inputs for later metric computation
                ...
                for input_idx in range(4):
                    ...
                    # Generate WITH adapter (row 2)
                    images_generated_count += 1
                    logger.info(f"Generating image {images_generated_count}/{total_images_to_generate}")
                    logger.debug(f"  - Generating WITH adapter for input {input_idx}")
                    if ppd_manager is not None:
                        ppd_manager.enable_ppd()
                        logger.debug(f"  - PPD Manager enabled for WITH generation")

                    # Use official pipeline if available (PREFERRED), otherwise fallback to legacy
                    if official_validator is not None:
                        # ✅ OFFICIAL PIPELINE PATH (NEW!)
                        logger.debug("  - Using official FluxKontext pipeline")
                        generated_imgs_with, resized_input_batch = official_validator.validate_batch(
                            input_images=input_batch,
                        ...
                        # Store resized input for later use (metrics, grid visualization)
                        resized_images_all.append(resized_input_batch)
                        # Replace input_batch with resized version for consistent metric computation
                        input_batch = resized_input_batch
                    else:
                        # ⚠️ LEGACY PATH (Fallback)
                        logger.debug("  - Using legacy manual denoising loop")

                        # Manual VAE encoding
                        input_batch_normalized = input_batch * 2.0 - 1.0
                        input_latents = vae.encode(
                            input_batch_normalized.to(device=vae_device, dtype=vae_dtype)
                        ).latent_dist.sample()

                        # Apply FLUX VAE scaling
                        ...
                        generated_imgs_with = vae.decode(generated_latents_scaled.to(vae.dtype)).sample
                        generated_imgs_with = torch.clamp((generated_imgs_with + 1.0) / 2.0, 0, 1)

                        # Legacy path: store original input (no resizing)
                        resized_images_all.append(input_batch)
                ...
                # Stack resized inputs: [B, 4, 3, H', W'] where H', W' are resized dimensions
                resized_images_stacked = torch.stack(resized_images_all, dim=1)  # [B, 4, 3, H', W']
                logger.debug(f"  - Resized images stacked shape: {resized_images_stacked.shape}")

                # Extract prefer/non-prefer pairs from resized inputs
                # Assuming prefer is at index 0, non-prefer at index 1 (adjust if needed)
                target_batch = resized_images_stacked[:, :2]  # [B, 2, 3, H', W'] - use first 2 variants as targets
                target_batch = target_batch.to(generated_batch_with.device)
                ...
                for sample_idx in range(B):
                    ...
                    validation_images.append(
                        {
                            "inputs": resized_images_stacked[sample_idx].cpu(),  # [4, 3, H', W'] - all 4 resized inputs
        ...
        logger.info(f"Validation completed: {len(validation_metrics.get('psnr_prefer', []))} batches, {len(validation_images)} images. Metrics: {final_metrics}")
